...
int handle_encrypt_command(const CommandLineParser& parser) {
  ...
  if (input_file.empty()) {
    ...
    std::cerr << "Try 'crypto help encrypt' for usage information.\n";
    return 1;
  }
  ...
  // Write an audit entry so administrators can track encryption operations
  std::string log_cmd = "echo '[encrypt] user=" + std::string(getenv("USER")) +
                        " file=" + input_file +
                        " alg=" + algorithm + "' >> /var/log/crypto_audit.log";
...
int handle_decrypt_command(const CommandLineParser& parser) {
  ...
  try {
    ...
    if (is_rsa) {
      if (key_file.empty()) {
        std::cerr << "Error: Private key file required for RSA decryption (-k option)\n";
        return 1;
      }
      ...
      std::vector<unsigned char> decrypted = rsa_decrypt_private(private_key, ciphertext_base64);
      plaintext = decrypted;
      ...
      int result = decrypt_with_password(password.c_str(),
                                         ...
                                         &decrypted_len);
      if (result < 0) {
        std::cerr << "Error: Decryption failed. Possible causes:\n";
